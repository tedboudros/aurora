...
from Helpers.constants import \
    ...
    DIRECTORIES_TO_WATCH_FOR_SERVER_RECOMPILATION
...
class ServerEventHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        seconds_since_last_compilation = (datetime.now() - self.last_ran_at).seconds

        if not event.is_directory:
            for dir_to_watch in DIRECTORIES_TO_WATCH_FOR_SERVER_RECOMPILATION:
                if event.src_path.find(
                        dir_to_watch) != -1 and seconds_since_last_compilation >= TIME_TO_ELAPSE_FROM_LAST_SERVER_COMPILATION:
                    # If the changed file is inside the directories we're watching for, refresh the server
                    self.server.refresh()
                    self.last_ran_at = datetime.now()